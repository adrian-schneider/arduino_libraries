...
Adafruit_mPCD8544::Adafruit_mPCD8544(int8_t SCLK, int8_t DIN, int8_t DC,
    int8_t CS, int8_t RST) : Adafruit_mGFX(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
Adafruit_mPCD8544::Adafruit_mPCD8544(int8_t SCLK, int8_t DIN, int8_t DC,
    int8_t RST) : Adafruit_mGFX(LCDWIDTH, LCDHEIGHT) {
  _din = DIN;
  _sclk = SCLK;
  _dc = DC;
  _rst = RST;
...
Adafruit_mPCD8544::Adafruit_mPCD8544(int8_t DC, int8_t CS, int8_t RST):
  Adafruit_mGFX(LCDWIDTH, LCDHEIGHT) {
...
void Adafruit_mPCD8544::drawPixel(int16_t x, int16_t y, uint16_t color) {
...
uint8_t Adafruit_mPCD8544::getPixel(int8_t x, int8_t y) {
  if ((x < 0) || (x >= LCDWIDTH) || (y < 0) || (y >= LCDHEIGHT))
    return 0;
...
void Adafruit_mPCD8544::begin(uint8_t contrast, uint8_t bias) {
  if (isHardwareSPI()) {
    // Setup hardware SPI.
    SPI.begin();
...
inline void Adafruit_mPCD8544::spiWrite(uint8_t d) {
  if (isHardwareSPI()) {
    // Hardware SPI write.
    SPI.transfer(d);
  }
...
bool Adafruit_mPCD8544::isHardwareSPI() {
  return (_din == -1 && _sclk == -1);
}

void Adafruit_mPCD8544::command(uint8_t c) {
...
void Adafruit_mPCD8544::data(uint8_t c) {
...
void Adafruit_mPCD8544::setContrast(uint8_t val) {
  if (val > 0x7f) {
    val = 0x7f;
  }
  command(PCD8544_FUNCTIONSET | PCD8544_EXTENDEDINSTRUCTION );
...
void Adafruit_mPCD8544::display(void) {
...
// clear everything
void Adafruit_mPCD8544::clearDisplay(void) {
